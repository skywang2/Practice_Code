...
//	vec3 ambient = ambientStrength * u_lightColor;
	vec3 ambient = u_lightMaterial.ambient * u_material.ambient;//使用material
...
//	vec3 diffuse = u_lightColor * diff;//===注意这个乘法，这是element-wise product===
	vec3 diffuse = u_lightMaterial.diffuse * (diff * u_material.diffuse);//===注意这个乘法，这是element-wise product===
...
//	vec3 specular = u_lightColor * specularStrength * spec;
	...
	vec3 specular = u_lightMaterial.specular * (spec * u_material.specular);
	
	//u_objectColor相当于公式里的反射率k
//	vec3 result = u_objectColor * (ambient + diffuse + specular);//光作用在物体上，用wise product表示，这只是数学上的一种表示，只是一种近似
	vec3 result = ambient + diffuse + specular;//物体颜色已经包含在material中
